# Rail-Transit-Practice-3 — 轨道交通模拟系统

软件暑期实训3 —— 基于 Python + Qt 的轨道交通运行模拟系统。

## 项目简介

本系统面向轨道交通运行模拟，实现了一个**可演示、可交互、可扩展**的轨道交通模拟系统。

系统模拟列车在线路上的运行过程，涵盖**多体车辆动力学、多列车调度、交路与折返、简化进路联锁、线路条件、信号约束、供电状态、车门联锁、运行日志与评价、线路可视化**等核心功能，支持手动/自动驾驶演示、调度组织、安全逻辑验证及运行评价。

✅ **当前状态：Web ATS 与桌面 GUI 可正常运行；最近一次完整测试为 344 项通过、7 项待修复。**

## 技术栈

| 技术 | 用途 |
|------|------|
| **Python 3.10+** | 主开发语言 |
| **PyQt5** | 桌面 GUI 框架，实现可视化界面与交互控制 |
| **FastAPI + Uvicorn** | Web ATS 后端，提供 REST API 与 WebSocket 实时通道 |
| **SQLite** | 线路数据库存储（支持 CRUD 编辑） |
| **xlrd** | 读取 Excel 格式的线路数据并导入 SQLite |
| **pytest** | 单元测试框架 |

## 功能模块

```
┌──────────────────────────────────────────────────────────────────┐
│                       轨道交通模拟系统                              │
├────────────┬──────────┬──────────┬──────────┬────────────────────┤
│  车辆仿真   │  轨道线路  │ 信号与调度 │  供电系统  │   可视化与日志      │
├────────────┼──────────┼──────────┼──────────┼────────────────────┤
│ · 多体动力学 │ · 站点数据 │ · 红黄绿灯 │ · 正常    │ · 状态仪表盘        │
│ · 逐车力计算 │ · 道岔分支 │ · 多列车   │ · 低压    │ · 驾驶控制面板       │
│ · 车钩力    │ · 限速区段 │ · 区段占用 │ · 断电    │ · 调度中心          │
│ · 牵引/制动 │ · 坡度区段 │ · 进路锁闭 │ · 故障恢复 │ · 线路全景图         │
│ · 自动驾驶  │ · 信号机  │ · 交路停站 │          │ · 速度/车钩力曲线     │
│ · 双向运行  │ · 站台侧  │ · 自动折返 │          │ · 运行日志与评价       │
│ · 车门联锁  │ · 数据库  │ · 调度命令 │          │ · 力分量明细表        │
└────────────┴──────────┴──────────┴──────────┴────────────────────┘
```

### 1. 车辆仿真模块 (`src/vehicle/`)

#### 多体动力学控制器（新版，当前主力）

`VehicleController` 是列车顶层控制器，管理编组状态、控制指令和仿真步进。每个仿真步内：

1. 控制指令（`throttle` / `brake_level`）经过一阶低通滤波，模拟牵引建磁和制动响应迟滞
2. 外部步长拆分为多个物理微步，保证车钩力和高速积分稳定性
3. 逐车查询所在位置的限速、坡度、曲线、隧道和黏着条件
4. 逐车计算 Davis 阻力、坡度阻力、隧道阻力、曲线阻力、牵引力、电制动力和摩擦制动力
5. 根据相邻车辆位置差、速度差和车钩参数计算车钩力
6. 聚合每节车合力，按等效质量更新单车速度、位置和加速度
7. 限速裁剪后生成 `ForceReport`，记录各力分量和限幅情况

| 模块 | 文件 | 说明 |
|------|------|------|
| 顶层控制器 | `vehicle_controller.py` | 编组管理、指令滤波、步进调度 |
| 动力学流水线 | `dynamics_pipeline.py` | 逐车力计算与状态积分 |
| 力计算 | `forces.py` | 牵引/制动/阻力/坡度等力分量 |
| 车钩力 | `coupler.py` | 相邻车辆间车钩力建模 |
| 自动驾驶 | `auto_drive.py` | 目标停车、巡航、紧急制动三段控制 |
| 环境接口 | `environment.py` | 天气、线路查询接口（`MockEnvironment`） |
| 枚举定义 | `enums.py` | 运行模式、载荷等级、车门侧、控制级位 |
| 力报告 | `force_report.py` | 动力学计算结果的数据结构 |
| UI 适配层 | `ui_adapter.py` | 将 `TrackData` 适配为 `ITrackQuery`，桥接新旧模块 |

#### 旧单质点模型（已废弃，保留兼容）

`VehicleModel` / `Controller` 为早期单质点实现，不再维护。Qt 前端已完全迁移至 `VehicleController`。

### 2. 轨道线路模块 (`src/track/`)

| 模块 | 文件 | 说明 |
|------|------|------|
| 线路数据结构 | `data.py` | `TrackData`、`Station`、`Platform`、`Segment`、`SpeedLimit`、`Gradient`、`Signal` |
| Excel 加载器 | `loader.py` | 从 Excel 读取线路数据，构建 `TrackData` |
| 数据库加载器 | `db_loader.py` | 从 SQLite 数据库加载线路数据 |
| 线路编辑器 | `editor.py` | 对 SQLite 中线路数据进行 CRUD 操作 |
| 车辆适配器 | `adapter.py` | `TrackDataAdapter`：将 `TrackData` 包装为 `ITrackQuery` 接口 |

**线路数据源切换**：程序默认加载 SQLite 数据库线路；界面仍保留「演示数据（备用）」用于离线诊断。

当前数据库线路包含 319 个区段、13 个车站、56 个站台和 97 架信号机。数据库中 KYL、FTN、FTD、QLZ 四站的里程字段为 `0`，加载器会根据所属站台区段中心在内存中补齐，原始 `railway.db` 不会被修改。

### 3. 信号系统模块 (`src/signal/`)

- **绿灯** — 允许正常运行
- **黄灯** — 允许运行，需降低速度（限速 10 m/s）
- **红灯** — 防护区段被其他列车占用时显示

信号计算逻辑：
1. 按运行方向对信号机排序
2. 防护区段被列车占用时显示红灯，前一架信号显示黄灯预告
3. 空闲区段统一显示绿灯，进路互斥由联锁系统保证
4. 列车接近红灯时切除牵引并自动制动，黄灯状态执行 10 m/s 限速
5. 支持上下行方向分别查询前方信号
6. 支持协议灯色码（0x01~0x30）归约为红/黄/绿三态
7. 有效限速 = `min(线路限速, 信号限速)`

### 4. 行车调度模块 (`src/dispatch/`)

调度模块在单机进程内统一管理多列车运行，每列车拥有独立的 `VehicleController`、`AutoDriveController` 和线路适配器。

| 能力 | 说明 |
|------|------|
| 多列车管理 | 支持加车、删车、列车编号和独立动力学状态 |
| 方向站台落位 | 数据库加车按上/下行站台 Seg 放置，不使用重叠绝对里程猜测线路 |
| 调度命令 | 严格按待发、运行、扣车、紧急停车等状态约束命令 |
| 统一进路 | 调度锁闭、车辆活动进路和自动驾驶目标共用同一方向站台拓扑 |
| 区段占用 | 根据整列车各车厢位置生成实时占用表 |
| 滚动进路 | 锁闭当前区段和前方区段，通过后自动滚动释放 |
| 冲突防护 | 目标区段被其他列车占用或锁闭时拒绝进路 |
| 列车间隔防护 | 沿 Seg 拓扑计算前车净间距，人工/自动驾驶均不可绕过 |
| 防撞保底 | 接近时预警，制动距离不足时强制制动，车体重叠时双车紧急制动 |
| 运营交路 | 按车站序列自动设置下一目标站和停站时间 |
| 自动折返 | 终端停车后换端，反转运行方向并继续返程 |

> 当前实现是适用于教学演示的单机简化调度与区段级联锁，不等同于完整联锁表、ATS 或 CBTC 系统。

列车间隔防护不依赖信号机或调度进路，因此即使列车处于人工驾驶，也会根据相对速度和制动距离施加安全制动。防护事件以 `WARNING`记录，车体重叠以 `CRITICAL` 碰撞事件记录并纳入运行评价。

### 5. 供电状态模块 (`src/power/`)

模拟供电条件对牵引能力的影响：

| 状态 | 牵引限制 |
|------|----------|
| 供电正常 | 100% 牵引能力 |
| 电压低 | 限制为 50% 牵引力 |
| 断电 | 禁止牵引 |
| 故障恢复 | 3 秒后恢复供电 |

### 6. 车门联锁模块 (`src/door/`)

`DoorInterlock` 保证停车、开门、发车符合安全规则：
- 车速 > 0.5 m/s 时禁止开门
- 车门未关时禁止发车
- 根据站台侧自动判断开左门/右门
- 兼容新旧车辆控制器

### 7. 网络通信模块 (`src/network/`)

提供与外部真实硬件的网络通信能力（独立线程，不阻塞 UI）：

| 子系统 | 协议 | 周期 | 用途 |
|--------|------|------|------|
| 车辆 UDP | UDP | 20ms | 收发车辆加速度、速度、距离 |
| 信号网关 | UDP | 250ms/100ms | 信号状态同步 |
| 司机台 PLC | TCP | 100ms | 司机操作输入、ATP 安全输出 |

> 网络模块已接入 Web ATS，默认使用 `config/network_config.py` 中的
> `MODE = "local"`。与实验室实体设备联调时切换为 `MODE = "lab_704"`，
> 并确认本机与司机台设备网络互通。

### 8. 可视化界面 (`src/ui/`)

| 组件 | 文件 | 说明 |
|------|------|------|
| 主窗口 | `main_window.py` | 应用主界面，Tab 切换、定时更新循环 |
| 状态仪表盘 | `dashboard.py` | 速度/限速/位置/坡度/信号/供电/车门/编组 |
| 控制面板 | `controls.py` | 牵引/制动/惰行/紧急制动/车门/载荷/模式切换 |
| 调度中心 | `dispatch_view.py` | 多列车列表、全线态势、交路设置和调度命令 |
| 线路全景图 | `track_view.py` | QGraphicsView 绘制轨道、道岔、信号机、列车位置 |
| 力学分析面板 | `charts.py` | 速度-时间曲线、车钩力曲线、逐车力分量明细表 |

### 9. 日志与评价模块 (`src/logger/`)

| 模块 | 文件 | 说明 |
|------|------|------|
| 事件记录器 | `recorder.py` | 结构化运行事件、组合筛选、CSV 实时落盘 |
| 运行评价器 | `evaluator.py` | 停车误差、最大速度、安全事件、综合得分与等级 |

#### Web ATS 综合评价口径

桌面端、日志模块和 Web 回放页面统一调用 `src/logger/evaluator.py`。
`GET /api/replay` 返回本次运行的分项评价，各指标均为 `0～100` 分。
综合分按安全性 30%、停车精度 20%、平稳性 20%、节能性 15%、准点性
10%、操作规范 5% 加权；未采集指标不参与计算，其余权重自动归一化。
等级划分为：`90～100` 优秀、`80～89.9` 良好、`60～79.9` 合格、低于
`60` 不合格。

| 指标 | 当前计算逻辑 | 说明 |
|------|--------------|------|
| 安全性 | `100 - 超速×5 - 闯红灯×20 - 紧急制动×10 - 碰撞×30` | 最低为 0 分 |
| 准点性 | `100 - abs(发车次数 - 到站次数)×10` | 当前是运行完整度代理指标，尚未接入计划时刻表 |
| 停车精度 | 暂未采集，返回 `null` | 不参与综合分 |
| 平稳性 | `冲动率得分×70% + 加速度得分×20% + 牵引/制动切换得分×10%` | 使用真实采样时间和 95% 分位值 |
| 节能性 | `min(100, 70 + 平均再生制动比例×30)` | 当前主要衡量再生能量利用率 |
| 操作规范 | `100 - Web 警告级失败命令数×5` | 最低为 0 分 |

平稳性先由相邻速度计算加速度，再由相邻加速度计算冲动率
`j = Δa / Δt`。冲动率 95% 分位不超过 `0.5 m/s³`、加速度绝对值
95% 分位不超过 `1.0 m/s²` 时不扣分；每分钟牵引/制动方向切换不超过
4 次时不扣分。折返换股、跳站、重置、状态切换和超过正常采样间隔的
非连续帧会被排除，避免单个数据跳点把整趟平稳性直接判为 0 分。

日志文件自动保存到 `logs/run_YYYYMMDD_HHMMSS_ffffff.csv`，每条事件包含：

| 字段 | 含义 |
|------|------|
| `timestamp` | 可复现的仿真相对时间 |
| `event_type` / `severity` | 事件类型与 `INFO` / `WARNING` / `CRITICAL` 等级 |
| `source` | `dispatch`、`signal`、`control`、`simulation`、`protection` 等来源 |
| `train_id` | 关联列车，用于多车筛选 |
| `entity_id` | 关联信号机、车站或区段 ID |
| `description` / `position_m` / `speed_m_s` | 事件说明及发生时运行状态 |

日志去重口径：信号机首次扫描只建立状态基线，仅在后续灯色变化时记录；持续超速从进入超速到恢复正常视为一次事件。日志面板会实时显示事件总数、警告数、严重事件数和运行评价。

可以通过 `Recorder.query(event_type=..., train_id=..., source=..., severity=...)` 按结构化字段组合筛选内存中的本次运行事件。

## 公共数据结构 (`src/common/`)

| 模块 | 说明 |
|------|------|
| `car_config.py` | 单车参数（质量、车长、牵引/制动特性）与车钩参数 |
| `car_state.py` | 单车运行时状态（位置、速度、加速度） |
| `consist.py` | 列车编组定义（预设：4M2T 六节编组） |
| `track_position.py` | 线路位置抽象（`TrackPosition`）与查询接口（`ITrackQuery`） |

## 项目结构

```
Rail-Transit-Practice-3/
├── resource/                   # 资源文件
│   ├── 线路数据(1).xls         # 原始线路 Excel 数据
│   ├── link_mainline.json      # 上下行主线 Link 链（校对后公里标）
│   ├── vision_alignment.json   # 视景边公里标、站台范围与开门侧标注
│   ├── ats_layout.json         # ATS 道岔与信号机布局
│   └── 初步目标需求分析.md      # 需求分析文档
├── data/                       # 运行时数据
│   └── railway.db              # SQLite 线路数据库
├── src/                        # 源代码
│   ├── main.py                 # 程序入口
│   ├── common/                 # 公共数据结构
│   │   ├── car_config.py       # 单车参数
│   │   ├── car_state.py        # 单车状态
│   │   ├── consist.py          # 列车编组
│   │   └── track_position.py   # 位置抽象与查询接口
│   ├── vehicle/                # 车辆仿真模块
│   │   ├── vehicle_controller.py  # 多体车辆顶层控制器
│   │   ├── dynamics_pipeline.py   # 逐车动力学计算流水线
│   │   ├── forces.py           # 牵引/制动/阻力计算
│   │   ├── coupler.py          # 车钩力计算
│   │   ├── auto_drive.py       # 自动驾驶控制器
│   │   ├── environment.py      # 环境查询接口
│   │   ├── enums.py            # 枚举定义
│   │   ├── force_report.py     # 动力学计算报告
│   │   ├── ui_adapter.py       # UI 适配层
│   │   ├── model.py            # (废弃) 旧单质点模型
│   │   └── controller.py       # (废弃) 旧控制器
│   ├── track/                  # 轨道线路模块
│   │   ├── data.py             # 线路数据结构
│   │   ├── loader.py           # Excel 数据加载
│   │   ├── db_loader.py        # SQLite 数据库加载
│   │   ├── editor.py           # 线路数据编辑器
│   │   ├── adapter.py          # TrackData → ITrackQuery 适配
│   │   ├── link_mainline.py    # 上下行主线 Link 链与坐标映射
│   │   ├── semantic_line.py    # 线路语义抽象（车站/区间/分支）
│   │   ├── ats_layout.py       # ATS 道岔与信号布局数据
│   │   └── route.py            # 进路数据结构
│   ├── signal/                 # 信号系统模块
│   │   └── system.py           # 信号状态逻辑
│   ├── dispatch/               # 多列车行车调度模块
│   │   ├── models.py           # 列车状态与运营交路模型
│   │   ├── train_manager.py    # 多列车生命周期管理
│   │   ├── interlocking.py     # 区段占用、进路锁闭与冲突检测
│   │   └── dispatch_manager.py # 调度命令、停站与折返状态机
│   ├── power/                  # 供电状态模块
│   │   └── supply.py           # 供电状态管理
│   ├── door/                   # 车门联锁模块
│   │   └── interlock.py        # 车门与站台联锁
│   ├── network/                # 网络通信模块（预留）
│   │   ├── manager.py          # 通信管理器
│   │   ├── udp_vehicle.py      # 车辆 UDP 通信
│   │   ├── signal_gateway.py   # 信号网关
│   │   ├── tcp_plc.py          # 司机台 PLC
│   │   ├── codec.py            # 编解码
│   │   └── constants.py        # 网络常量
│   ├── logger/                 # 日志与评价模块
│   │   ├── recorder.py         # 事件记录
│   │   └── evaluator.py        # 运行评价
│   ├── web/                     # Web ATS 后端
│   │   ├── app.py               # FastAPI 入口
│   │   └── simulation.py        # 仿真运行时（状态管理、快照）
│   └── ui/                     # 可视化界面
│       ├── main_window.py      # 主窗口
│       ├── dashboard.py        # 状态仪表盘
│       ├── controls.py         # 控制面板
│       ├── dispatch_view.py    # 调度中心界面
│       ├── track_view.py       # 线路全景图
│       ├── semantic_track_view.py  # 运营语义线路图
│       └── charts.py           # 速度曲线与力分量表
├── scripts/                     # 数据导入与维护脚本
│   ├── import_to_db.py          # Excel → SQLite 导入
│   └── import_ats_layout.py     # ATS 道岔/信号布局导入
├── tests/                      # 单元测试（344 个，全部通过）
│   ├── test_vehicle.py
│   ├── test_controller.py
│   ├── test_dynamics_pipeline.py
│   ├── test_forces.py
│   ├── test_auto_drive.py
│   ├── test_coupler.py
│   ├── test_consist.py
│   ├── test_car_config.py
│   ├── test_car_state.py
│   ├── test_environment.py
│   ├── test_force_report.py
│   ├── test_track_position.py
│   ├── test_track.py
│   ├── test_signal.py
│   ├── test_recorder.py
│   ├── test_editor.py
│   ├── test_dispatch.py
│   ├── test_database_view.py
│   ├── test_vehicle_ui_adapter.py
│   └── __init__.py
├── web-ats/                     # Web ATS 前端
│   ├── index.html               # 单页应用（含全部页面模板与渲染逻辑）
│   └── scripts/                 # 前端脚本（按模块拆分）
├── output/                      # 运行输出
├── .gitignore
├── README.md
└── requirements.txt
```

## 快速开始

### 环境要求

- Python 3.10+
- pip

### 安装依赖

```bash
pip install -r requirements.txt
```

### 运行程序

启动桌面 Qt 程序：

```bash
python -m src.main
```

### 运行 Web ATS

Web ATS 必须由 FastAPI 后端提供页面和实时线路数据，不能直接双击
`web-ats/index.html`，也不能只使用普通静态文件服务器打开。

首次运行先安装项目依赖：

```bash
pip install -r requirements.txt
```

仅在当前电脑使用时启动：

```bash
python -m uvicorn src.web.app:app --host 127.0.0.1 --port 8765
```

浏览器访问：

```text
http://127.0.0.1:8765/
```

如果需要让同一局域网内的其他电脑访问，服务端应监听全部网络接口：

```bash
python -m uvicorn src.web.app:app --host 0.0.0.0 --port 8765
```

其他电脑使用服务端的局域网 IP 访问，例如：

```text
http://192.168.1.20:8765/
```

若页面一直显示“线路拓扑加载中”，依次检查以下接口：

```text
http://127.0.0.1:8765/api/health
http://127.0.0.1:8765/api/snapshot
```

- `/api/health` 应返回 `"ok": true`。
- `/api/snapshot` 应包含 `line`、`line.directions.up` 和
  `line.directions.down`。
- 局域网访问时，将上述地址中的 `127.0.0.1` 替换为服务端 IP，并确认
  Windows 防火墙允许 TCP 端口 `8765`。
- 修改或拉取前端代码后，应重启 Uvicorn 并刷新浏览器页面。

Web ATS 当前支持：

- WebSocket 100 ms 实时快照，断线时自动降级为 HTTP 轮询；
- 多列车调度、命令结果与拒绝原因、紧急停车确认；
- 运营图与信号图双视图，独立缩放（滚轮）与平移（拖拽）；
- 主线信号机实时渲染（红/黄/绿），带竖线与轨道连接，悬停显示详情；
- 可点击 Link 区段详情、列车和信号图层切换；
- 上下行双线独立显示，占用/锁闭/空闲三色区分；
- 正常、低电压、断电、占用冲突、低黏着和通信中断六种场景；
- 500 ms 采样的运行回放、速度曲线、事件统计和基础安全评分；
- 实际接口连接状态、活动报警统计和 CSV 运行日志导出。

#### 司机台实时输入与硬件输出

Web 页面入口：

```text
接口与网络 → 联调连接 → 司机台联调
```

点击“联调连接”后，Web ATS 启动外部网络模块。司机台 PLC 默认连接
`192.168.100.123`，主端口为 `8001`，`8002`、`8003` 为备用端口。PLC
每约 `100 ms` 发送一帧 46 字节输入报文；上位机按需下发 28 字节输出报文。
数据链路如下：

```text
司机台 PLC
→ src/network/tcp_plc.py 接收
→ src/network/codec.py 解析
→ SimulationRuntime.plc_data
→ WebSocket 实时快照
→ Web 司机台联调页面
```

页面实时展示方向手柄、主控手柄、牵引/制动级位、ATO、紧急制动、
司机钥匙、车门模式和照明状态，并可查看最近 PLC 原始 HEX 报文。正常
情况下页面刷新频率约为 10 次/秒；超过通信时效后会显示未连接或最近
接收时间。

方向手柄按照接口协议使用 WORD 枚举：

```text
0x0000 = 零位
0x0001 = 向前
0x0002 = 向后
```

司机台输出分为两类：

1. **显示屏/仪表盘自动输出**：网络连接后，后端按约 `100 ms` 周期自动
   生成状态报文；网络屏为 `192.168.100.121:8888`（570 字节），信号屏
   为 `192.168.100.122:9999`（68 字节）。
2. **PLC 硬件灯与 ATP 输出**：通过 `POST /api/plc/output` 下发。接口既
   支持 `atp_safe_out` 数值，也支持高压接触器、制动缓解、车门关闭、
   网络故障、ATO 可用/激活和自动折返等布尔输出状态。

当前“司机台联调”页面已经提供 ATP 安全输出值（`0～65535`）发送框；
独立硬件灯的后端输出能力已经具备，但 Web 端尚未提供逐灯开关面板。
未启动网络时发送 ATP 输出会返回明确的失败信息，不会显示虚假成功。

相关接口：

```text
POST /api/network/start
POST /api/network/stop
GET  /api/plc/output
POST /api/plc/output
```

场景与回放接口：

```text
POST /api/scenarios/{scenario_id}
GET  /api/replay
```

可用的 `scenario_id` 为 `normal`、`low_voltage`、`power_outage`、
`occupancy_conflict`、`low_adhesion` 和 `communication_outage`。切换场景时
会清除上一个场景的注入状态，避免多种故障意外叠加。

### 调度中心演示

1. 打开「调度中心」，输入车号并选择上行/下行和加入车站；界面会显示对应站台 Seg。
2. 加车成功后系统自动选中新车，并同步为「运行仿真」当前受控列车。
3. 为当前调度对象设置运营交路，再点击「发车」。
4. 扣车后先解除扣车回到待发，再次点击发车才能运行。
5. 紧急停车后必须等待列车完全停稳，才能解除紧急状态并重新发车。
6. 列车到达终端站后会自动换端，并按反方向继续运行。

调度锁闭进路、车辆线路适配器和自动驾驶使用同一组有序 Seg；例如数据库线路下行 `GGZ → FSP` 的目标站台为 `Seg 51`，不会再按重叠绝对里程误选 `Seg 235`。

`1车` 默认保持人工驾驶，只有在调度中心为其设置交路并发车后，才切换为调度自动运行。

### 在运行仿真中控制多辆车

1. 先在「调度中心」把列车加入不同的空闲车站区段。
2. 回到「运行仿真」，在顶部“当前受控列车”下拉框选择目标列车。
3. 仪表盘、驾驶按钮、车门、编组和力学报告会切换到该列车。
4. 点击“手动”或直接点击牵引/制动，会把该列车切换到人工驾驶状态。
5. 点击“自动”可执行单站目标停车；设置交路并由调度发车则使用完整交路模式。

运营线路图和调度态势图均会实时显示所有列车。红色表示占用，橙色/黄色表示进路锁闭，信号灯色随闭塞状态刷新。

### 关于 libpng iCCP 警告

项目内两张 PNG 预览图均不包含 `iCCP` 或 `sRGB` 配置块，程序运行也不读取它们。若控制台仍出现 `libpng warning: iCCP: known incorrect sRGB profile`，通常来自本机 PyQt/IDE 插件自带图片，而不是仿真数据或界面逻辑；该信息不会影响运行。建议直接执行 `python -m src.main` 复核，或重装当前 Python 环境中的 PyQt5 资源包。

### 运行测试

```bash
pytest tests/ -v
```

## 已知问题

### 1. 信号防护制动力偏弱

当前信号/进路阻塞时施加 `brake(0.7)`，从 85 km/h 减速到 0 需要约 1.8 km，可能无法在前方占用段之前停住。需要评估是否提升制动级位或增加更远的前瞻距离。

### 2. 滚动进路窗口较小

调度滚动进路每次仅锁闭当前段 + 前一段（2 段），列车无法提前锁定更远区段。虽不影响运行安全（联锁与间隔防护为保底），但不利于提前预告信号状态。

### 3. 部分站台原始数据方向码不一致

GGZ、LLQ、GTG 三个站的站台方向码在原始 Excel 中与其他 10 站采用不同约定（`0x55`/`0xAA` 的上下行含义相反），已在数据库层面手动修正。如需重新导入原始 Excel，应优先按接口协议（`0x55=上行, 0xAA=下行`）统一解析，并对这三个站做特殊处理。

### 4. Web ATS 信号机仅在主线显示

侧线/分支上的信号机（约 70 架）因不属于 link_mainline 链路，暂未在 Web 线路图上渲染。

## 需求进度

| 优先级 | 内容 | 状态 |
|--------|------|------|
| **Must** | 车辆运行、线路站点、限速、坡度、手动控制、状态显示、日志记录 | ✅ 完成 |
| **Must** | 多体车辆动力学（逐车计算、车钩力） | ✅ 完成 |
| **Should** | 自动停站、信号约束、供电异常、车门联锁 | ✅ 完成 |
| **Should** | 线路全景可视化、力学分析面板 | ✅ 完成 |
| **Should** | SQLite 数据库线路加载与编辑 | ✅ 完成 |
| **Should** | 多列车调度、区段占用、滚动进路与冲突防护 | ✅ 完成 |
| **Should** | 交路停站、终端换端和自动折返 | ✅ 完成 |
| **Could** | 天气影响、载荷模式、黏着控制 | ✅ 完成 |
| **Could** | 网络通信接口（UDP/信号网关/PLC） | 🔧 框架预留 |
| **Could** | Web ATS 实时监控、场景注入与基础回放 | ✅ 完成 |
| **Won't** | 完整 CBTC、完整联锁表、真实供电网络 | — |

## 后续扩展方向

- **网络模式接入**：将 `NetworkManager` 与实际外部硬件联调
- **Web 工程化扩展**：继续将页面模板和图形渲染拆分为独立模块
- **CBTC 移动闭塞**：从简化固定闭塞升级为移动闭塞
- **完整联锁表**：支持真实联锁进路逻辑
- **多工作站调度**：增加调度员、车站值班员权限与控制权转换
