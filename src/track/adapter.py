...
class TrackDataAdapter(ITrackQuery):
    ...
    def advance_position(self, pos: TrackPosition, distance: float) -> TrackPosition:
        ...
        岔口处不可用绝对里程线性累加 + from_absolute，否则会误入侧线死胡同。
        """
        if distance == 0:
            return pos
    ...
    def find_nearest_lateral_fork(self, from_seg_id: int,
                                 direction: int = 1,
                                 max_hops: int = 80) -> Optional[int]:
        """从当前区段沿运行方向寻找最近的侧向岔口区段 ID。"""
        seg_map = self._td._seg_map
        if from_seg_id not in seg_map:
            return None
        forward = direction >= 0
        cur = from_seg_id
        visited: Set[int] = set()
        for _ in range(max_hops):
            ...
            if forward:
                if seg.end_lateral > 0 and seg.end_lateral != 65535:
                    return cur
                nxt = self._pick_forward_exit(seg)
            else:
                if seg.start_lateral > 0 and seg.start_lateral != 65535:
                    return cur
                nxt = self._next_backward(seg)
            if nxt is None or nxt not in seg_map:
                break
            cur = nxt
        return None
